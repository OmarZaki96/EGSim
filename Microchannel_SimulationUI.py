...
from Solver_microchannel import solver
from Results_tab import ResultsTabsWidget_Microchannel
import time
import datetime
from GUI_functions import save_Microchannel_results_excel, read_Microchannel, write_Microchannel, copy_file
import appdirs
from MicrochannelUI import MicrochannelWindow
from copy import deepcopy

FROM_Microchannel_Simulation,_ = loadUiType(os.path.join(os.path.dirname(__file__),"ui_files/Microchannel_Simulation.ui"))

class values():
    pass

class Microchannel_Simulation_Window(QDialog, FROM_Microchannel_Simulation):
    def __init__(self, parent=None):
        super(Microchannel_Simulation_Window, self).__init__(parent)
        self.setupUi(self)
    ...
    def results_creator(self):
        ...
        self.Results_tabs_widget = ResultsTabsWidget_Microchannel(HX=self.result_HX)
    ...
    def export_to_excel(self):
            path = QFileDialog.getSaveFileName(self, caption='Save Microchannel results',directory=load_last_location(),filter="Excel file (*.xlsx);;All files (*.*)")[0]
            if path != "":
                save_last_location(path)
                if path[-5:].lower() != ".xlsx":
                    path = path+".xlsx"
                try:
                    succeeded = save_Microchannel_results_excel(self.result_HX,path)
                    if not succeeded:
                        raise
                    msg = QMessageBox()
    ...
    def validate(self):
        ...
        if self.Component_List.currentRow() == -1:
            return (2, "Please choose a microchannel heat exhanger")
        ...
        self.HX = self.parent.Microchannel_list[self.Component_List.currentRow()][1]
        
        result = check_microchannel(self.HX)
        if not result[0]:
            return (2,result[1])
        return (1,)
        ...
        if selected_items:
            ...
            if reply == QMessageBox.Yes:
                try:
                    ...
                    msg = QMessageBox()
                    msg.setIcon(QMessageBox.Information)
                    msg.setText("Microchannel heat exchanger was deleted successfully")
                    msg.setWindowTitle("Microchannel heat exchanger was deleted!")
                    msg.setStandardButtons(QMessageBox.Ok)
                    msg.exec_()                
                except:
                    msg = QMessageBox()
                    msg.setIcon(QMessageBox.Warning)
                    msg.setText("Microchannel heat exchanger Could not be deleted")
                    msg.setWindowTitle("Microchannel heat exchanger was not deleted!")
                    msg.setStandardButtons(QMessageBox.Ok)
                    msg.exec_()
        else:
            msg = QMessageBox()
